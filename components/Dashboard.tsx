...
const Dashboard: React.FC<Props> = ({ result, respondentData }) => {
  ...
  return (
    <div className="space-y-12 animate-fade-in pb-20 print:space-y-8 print:pb-0 bg-white sm:bg-transparent">
      ...
      <div className="flex justify-between items-end no-print">
        <div>
          <h1 className="text-3xl font-black text-slate-900">Dashboard Estratégico</h1>
          <p className="text-slate-500 mt-1">Visão integrada de meio ambiente e governança.</p>
        </div>
        <button 
            onClick={() => window.print()} 
            className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3 rounded-lg font-bold transition-colors shadow-lg hover:shadow-xl"
        >
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm print:border-2 print:border-slate-800 print:shadow-none print:rounded-lg">
        <div className="flex flex-wrap gap-8 text-sm md:text-base items-center">
            <div className="flex-1">
                <span className="block text-xs uppercase font-bold text-slate-400 mb-1">Responsável</span>
                <span className="font-bold text-slate-900 text-lg">{respondentData?.name || '-'}</span>
            </div>
            <div className="flex-1">
                <span className="block text-xs uppercase font-bold text-slate-400 mb-1">Setor</span>
                <span className="font-bold text-slate-900 text-lg">{respondentData?.sector || '-'}</span>
            </div>
            ...
            <div className="ml-auto">
                <span className="block text-xs uppercase font-bold text-slate-400 mb-1 text-right">Status Geral</span>
                <span className={`px-4 py-1.5 rounded-full text-sm font-black uppercase border
      ...
      {/* Critical Vision Section */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 print:grid-cols-2 print:gap-4 print:break-inside-avoid">
        {/* Gauge Chart */}
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 flex flex-col items-center justify-center text-center relative overflow-hidden print:border print:p-4 print:border-slate-300">
            <h3 className="text-lg font-bold text-slate-800 mb-4 z-10">Índice de Maturidade</h3>
            <div className="relative w-64 h-64 z-10 print:w-48 print:h-48">
                ...
                <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-0 text-center">
                    <span className="text-5xl font-black text-slate-900 block print:text-4xl" style={{ color: getScoreColor(result.percentage) }}>
                        {result.percentage.toFixed(0)}%
                    </span>
                    <span className="text-xs font-bold text-slate-400 uppercase mt-2 block">Performance</span>
                </div>
            </div>
        ...
        {/* Radar Chart */}
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 lg:col-span-2 print:col-span-1 print:border print:border-slate-300">
            <h3 className="text-lg font-bold text-slate-800 mb-4">Raio-X dos Eixos Temáticos</h3>
      ...
      <div className="space-y-8 print:space-y-6">
        <div className="flex items-center gap-3 mb-6 print:break-before-page print:mt-8">
            ...
            <h2 className="text-2xl font-black text-slate-900">Plano de Ação Cronológico</h2>
        </div>
                ...
                <div key={timeframe} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden print:border print:border-slate-300 print:shadow-none mb-6 print:break-inside-avoid">
                    ...
                    <div className="divide-y divide-slate-100 print:divide-slate-200">
                        {timeActions.map((action, idx) => (
                            <div key={idx} className="p-6 hover:bg-slate-50 transition-colors print:p-4">
                                <div className="flex flex-col md:flex-row justify-between mb-2">
                                    <span className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1 md:mb-0 bg-slate-100 px-2 py-0.5 rounded w-fit border border-slate-200">{action.category}</span>
                                    {action.priority === 'Alta' && <span className="text-xs font-bold text-red-600 bg-red-100 px-2 py-0.5 rounded w-fit border border-red-200">PRIORIDADE MÁXIMA</span>}
                                </div>
                                ...
                                <div className="flex flex-wrap gap-4 text-xs font-medium text-slate-500 bg-slate-50 p-3 rounded-lg border border-slate-100 print:bg-white print:border-slate-200">
