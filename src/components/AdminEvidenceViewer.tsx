...
const AdminEvidenceViewer: React.FC<Props> = ({ evidence }) => {
  ...
  const hasFile = !!evidence.fileUrl;
  const hasComment = !!evidence.comment;
  ...
  return (
    <>
      {/* Comment Section */}
      {hasComment && (
         <div className="bg-white p-3 rounded-lg border border-slate-100 shadow-sm mb-2 text-sm">
             <div className="flex items-center gap-2 mb-1 text-slate-400 font-bold text-[10px] uppercase tracking-wide">
                 <span>📝 Comentário / Justificativa</span>
             </div>
             <p className="text-slate-700 italic leading-relaxed">"{evidence.comment}"</p>
         </div>
      )}

      {/* File Section */}
      {hasFile && (
          <div className="flex items-center gap-2 bg-slate-100 border border-slate-200 p-2 rounded-lg">
             <div className="bg-white p-2 rounded border border-slate-200 shadow-sm text-xs font-bold text-slate-500 uppercase">
                {evidence.fileName?.split('.').pop()}
             </div>
             <div className="flex-1 min-w-0">
                <p className="text-xs font-bold text-slate-700 truncate" title={evidence.fileName}>{evidence.fileName}</p>
